...
// ─── Main orchestrator ────────────────────────────────────────────
async function runMonthlyPulls(userId) {
  const customers = [
    {
      id: userId ?? process.env.MARCUS_USER_ID,
      firstName: 'Marcus',
      lastName: 'Rivera',
...
async function processCustomer(customer) {
  ...
  try {
    ...
    const { rows: [newSnap] } = await client.query(`
      ...
      ON CONFLICT (customer_id, pull_date) DO UPDATE SET
        score = EXCLUDED.score,
        score_type = EXCLUDED.score_type,
        reason_code_1 = EXCLUDED.reason_code_1,
        reason_code_2 = EXCLUDED.reason_code_2,
        reason_code_3 = EXCLUDED.reason_code_3,
        reason_code_4 = EXCLUDED.reason_code_4,
        active_duty_indicator = EXCLUDED.active_duty_indicator,
        loans_in_collection_count = EXCLUDED.loans_in_collection_count,
        loans_charged_off_count = EXCLUDED.loans_charged_off_count,
        loans_in_collection_amount = EXCLUDED.loans_in_collection_amount,
        loans_charged_off_amount = EXCLUDED.loans_charged_off_amount,
        open_lines = EXCLUDED.open_lines,
        past_due_lines = EXCLUDED.past_due_lines,
        total_open_balance = EXCLUDED.total_open_balance,
        total_past_due_amount = EXCLUDED.total_past_due_amount,
        total_inquiries_24h = EXCLUDED.total_inquiries_24h,
        total_inquiries_7d = EXCLUDED.total_inquiries_7d,
        total_inquiries_30d = EXCLUDED.total_inquiries_30d,
        total_inquiry_clusters_24h = EXCLUDED.total_inquiry_clusters_24h,
        temp_account_record = EXCLUDED.temp_account_record,
        is_no_hit = EXCLUDED.is_no_hit,
