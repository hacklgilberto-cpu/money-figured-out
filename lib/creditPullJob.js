...
const { pullWithRetry } = require('../services/clarityService');

// ─── Schedule: 2 AM on the 1st of every month ─────────────────────
cron.schedule('0 2 1 * *', runMonthlyPulls, { timezone: 'America/New_York' });

// ─── Main orchestrator ────────────────────────────────────────────
async function runMonthlyPulls() {
  console.log('[creditPullJob] Starting monthly pull run', new Date().toISOString());

  // Target active customers only (score < 740 or have had a prior hit)
  // Adjust the WHERE clause to match your customer table structure
  const { rows: customers } = await pool.query(`
    SELECT c.id, c.first_name AS "firstName", c.last_name AS "lastName",
           c.ssn_encrypted AS ssn, c.dob, c.address, c.city, c.state, c.zip
      FROM customers c
     WHERE c.is_active = TRUE
       AND (
         c.last_vantage_score < 740
         OR EXISTS (
           SELECT 1 FROM credit_snapshots cs
            WHERE cs.customer_id = c.id
              AND cs.is_no_hit = FALSE
         )
       )
     ORDER BY c.id
  `);

  console.log(`[creditPullJob] Processing ${customers.length} customers`);
  ...
  for (const customer of customers) {
    try {
      ...
      failed++;
      console.error(`[creditPullJob] Failed for customer ${customer.id}:`, err.message);
    }
    // Throttle: 200ms between requests to stay within Clarity rate limits
    await sleep(200);
  }

  console.log(`[creditPullJob] Done. Succeeded: ${succeeded} | Failed: ${failed}`);
}
    ...
    if (changes) {
      ...
      const { rows: [changeRow] } = await client.query(`
        INSERT INTO credit_score_changes (
          ...
          score_prev, score_curr, score_delta,
      ...
      `, [
        ...
        changes.scorePrev, changes.scoreCurr, changes.scoreDelta,
