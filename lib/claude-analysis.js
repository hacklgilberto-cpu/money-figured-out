...
- For mobile phone plan renegotiations: always name Public Mobile (publicmobile.ca) as the specific lower-cost alternative. Give the exact monthly savings.
- For coffee spending (Starbucks etc.): reference Tim Hortons as the Canadian comparison. Give a specific monthly savings number.
- For delivery apps (Uber Eats, DoorDash, SkipTheDishes): group them into ONE single cutThisFirst item with combined monthly total. Never list delivery apps separately.
- For streaming subscriptions: group ALL streaming services into ONE single cutThisFirst item. In the merchant name field, list services separated by commas NOT by plus signs (e.g. "Netflix, Crave, Disney+" not "Netflix + Crave + Disney+") to avoid confusing punctuation like "Disney+ + Crave".
- For renegotiate actions: always name the specific URL or phone number. Never say only "call your provider".
...

{
  ...
  "projection": {
    ...
    "oneLineSummary": "<one sentence about monthly savings growth. Never mention net worth, never use negative numbers. Example: Follow this plan and you go from saving $92/mo to $1,314/mo within 12 months. Under 20 words.>",
