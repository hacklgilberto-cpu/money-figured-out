...
// Pave/Plaid merchant names come in various forms:
//   "FLORIDA POWER AND LIGHT", "Florida Power & Light", "FPL"
// We strip to lowercase alpha for fuzzy matching.
const normalize = (name) =>
  (name || '').toLowerCase().replace(/[^a-z0-9]/g, '');

// Known aliases — maps common transaction names to canonical biller names.
// Extend this as more billers are added to playbook_cards.
const ALIASES = {
...
// Returns: { billerCards: [...], statePrograms: [...], localResources: [...] }
export async function matchPlaybooks(merchantNames, userState = 'FL') {
  // Resolve all merchant names to canonical biller names
  const resolved = [...new Set(
    merchantNames.map(resolveBillerName)
  )];

  // Pull all active playbook cards for this state + national (US)
  ...
  for (const card of rows) {
    if (card.biller_name === '_STATE_PROGRAM') {
      ...
      // Check if this card matches any of the user's merchants
      const cardNorm = normalize(card.biller_name);
      const matched = resolved.some(r => {
        const rNorm = normalize(r);
        return rNorm === cardNorm
          || rNorm.includes(cardNorm)
          || cardNorm.includes(rNorm);
      });

      if (matched) {
        billerCards.push(card);
      }
    }
  }
...
// Produces the text block that goes into the AI system prompt.
  ...
  if (mode === 'RED' || mode === 'ORANGE') {
    ...
    if (billerCards.length > 0) {
      ...
      for (const card of billerCards) {
        ...
        if (card.protection_rule) {
          lines.push(`    Protection: ${card.protection_rule}`);
        }
        lines.push('');
      }
    }
    ...
    lines.push('USER IS IN STABLE MODE — no survival playbooks needed.');
    lines.push('Focus on habit building, credit path, and optimization.');
  }

  return lines.join('\n');
}

// ── Seed a mock forecast for Marcus (demo) ──────────────────────
// Call this from the demo API to populate user_forecasts for Marcus.
export async function seedMarcusForecast(userId) {
  ...
  const income = 1640; // next paycheck
  const daysToPayday = 5;
  const gap = currentBalance - totalBills + income;

  // Marcus is in ORANGE: tight, bills coming, thin buffer
  // With $145 and $540.48 in bills over 14 days, he's short before payday
  // But the paycheck on day 5 covers most of it — except the AT&T on day 4
  const mode = 'ORANGE';
  const verdict = `You have $145. Your AT&T bill ($85) hits in 4 days — before your paycheck arrives on day 5. After AT&T, you'll have $60 for one day. Your next check ($1,640) lands the day after, then Progressive ($142) and Honda ($287) hit in the following week. You'll be tight but covered if the paycheck lands on time.`;
  ...
  `, [
    userId, today, currentBalance, totalBills, income,
    gap, 85, daysToPayday, mode,
    JSON.stringify(billsDetail), verdict,
  ]);
}
